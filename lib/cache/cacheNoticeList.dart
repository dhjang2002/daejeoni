...
class CacheNoticeList extends CacheBase{
  CacheNoticeList() : super();
  ...
  Future<List<ItemNotice>> geItems(BuildContext context, RequestParam param) async {
    List<ItemNotice> items = [];
    Map<String, dynamic> req = param.getMapParam();
    req.addAll({"board_id":"board001"});
    ...
    await Remote.apiPost(
        ...
        onResult: (dynamic data) {
          ...
          if (data['status'].toString() == "200")
          {
            ...
            if(content!=null) {
              items = ItemNotice.fromSnapshot(content);
            } else {
              items = [];
            }
          }
